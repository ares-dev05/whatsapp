<template>
  <div @contextmenu="openContextMenu" @click="closeContextMenu">
    ...
    >
      ...
      <span class="align-right" v-if="icons.network === 'wifi'"
        ><i class="material-icons">wifi</i></span
      >
      <span class="network align-right" v-else-if="icons.network === 'lte'"
        >LTE</span
      >
      <span class="network align-right" v-else-if="icons.network === '4G'"
        >4G</span
      >
      <span class="network align-right" v-else-if="icons.network === '3G'"
        >3G</span
      >
          ...
          <div
            :class="[
              indx === 1
                ...
                : 'fourth-bar',
              'bar',
              icons.signalStrengt < indx ? 'invalid' : '',
            ]"
            v-for="indx in 4"
            :key="'network_strength_' + indx"
          ></div>
        </div>
      </span>
      ...
      <div class="whatsapp-chat-send" style="background: #f6f6f6">
        <div class="whatsapp-chat-send__write">
          <span>
            <img
              src="images/add.png"
              style="height: 23px; margin-top: 5px; padding-left: 10px"
            />
          </span>
          <input type="text" placeholder="" />
          <img
            src="images/note.png"
            style="height: 20px; position: absolute; right: 107px"
          />
          <ul>
      ...
      <div v-else-if="contextmenuValue === 'headbar-context'">
        <ul class="menu-list">
          <li class="menu-item" v-if="icons.battery !== 'full'">
            <button @click="icons.battery = 'full'" class="menu-button">
              <i class="material-icons">battery_full</i>Batterie Anzeigen
            </button>
          </li>
          <li class="menu-item" v-if="icons.battery !== ''">
            ...
            <input
              ...
              style="width: 100%"
              @input="rerenderBattery"
              v-model="icons.batteryPercent"
              type="range"
              min="1"
              max="100"
            />
        ...
        <ul class="menu-list">
          <li class="menu-item" v-if="icons.network !== 'wifi'">
            <button @click="icons.network = 'wifi'" class="menu-button">
              <i class="material-icons">wifi</i>WLAN Anzeigen
            </button>
          </li>
          <li class="menu-button" v-if="icons.network !== 'lte'">
            <button @click="icons.network = 'lte'" class="menu-button">
              <i class="material-icons">lte_mobiledata</i>LTE Anzeigen
            </button>
          </li>
          <li class="menu-button" v-if="icons.network !== '4G'">
            <button @click="icons.network = '4G'" class="menu-button">
              <i class="material-icons">4g_mobiledata</i>4G Anzeigen
            </button>
          </li>
          <li class="menu-button" v-if="icons.network !== '3G'">
            <button @click="icons.network = '3G'" class="menu-button">
              <i class="material-icons">3g_mobiledata</i>3G Anzeigen
            </button>
          </li>
        </ul>
        <ul class="menu-list">
          <li class="menu-item" v-if="icons.signal !== 'on'">
            <button @click="icons.signal = 'on'" class="menu-button">
              <i class="fa fa-signal"></i>Signal Anzeigen
            </button>
          </li>
          <li class="menu-item" v-if="icons.signal !== 'off'">
            <label for="signalStrengtInput">Signal Stärke</label><br />
            <input
              id="signalStrengtInput"
              style="width: 100%"
              v-model="icons.signalStrengt"
              type="range"
              min="0"
              max="4"
            />
          ...
          <li class="menu-button" v-if="icons.signal !== 'off'">
            <button @click="icons.signal = 'off'" class="menu-button">
              <i class="material-icons">visibility_off</i>Signal ausblenden
            </button>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    ...
    rerenderBattery() {
      let pn = parseInt(this.icons.batteryPercent);
      if (!isNaN(pn)) {
        let pnd = pn / 100;
        if (pnd >= 1) {
          pnd = 1;
